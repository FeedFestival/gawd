...
public class HexMapGenerator : MonoBehaviour
{
    ...
    public Dictionary<int, Dictionary<int, Hex>> _hexes = new Dictionary<int, Dictionary<int, Hex>>();
    ...
    internal void AddEdgeHex()
    {
        ...
        _hexUnsetEdge.Enqueue(new Coord() { Y = y, X = x, Pole = coord.Pole });
        addToHexes(hex);
    }

    private void addToHexes(Hex hex)
    {
        if (_hexes.ContainsKey(hex.Y) == false)
        {
            _hexes.Add(hex.Y, new Dictionary<int, Hex>());
        }
    ...
    private Hex getNewHex(int y, int x, Vector3 pos)
    {
    ...
    private Coord getUnsetNeighborCoords(ref Hex currentHex)
    {
    ...
    private Coord findCoordsIfTheyDontExist(Coord? coord, ref Hex currentHex)
    {
    ...
    internal static bool DoesHexExist(Dictionary<int, Dictionary<int, Hex>> hexes, int y, int x)
    {
